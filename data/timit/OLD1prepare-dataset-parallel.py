    ...
    for feature_file, i in zip(utterance_filenames, np.arange(len(utterance_filenames))):
        ...
        # Load utterance frames with features
            ...
            for phone in range(tot_phones):
                frames = [k for k in range(tot_frames) \
                            if frame_start_points[k] >= expr_phones.loc[phone, "start"] \
                                and frame_start_points[k] < expr_phones.loc[phone, "end"]]

                frame_endings = frame_start_points[frames] + frame_size

                [labels.append(int(expr_phones.loc[phone, "type"] in vowels)) \
                            for frame_ending in frame_endings \
            ...
            """ In case there are incomplete frames at the end of utterance
                then append some zeroes, check if allowLastFrameIncomplete = 0
                in cFramer component. """
            last_column = expr_phones.columns.get_loc("type")
            [labels.append(int(expr_phones.iloc[-1, last_column] in vowels)) \
                            for incomplete_frame in np.arange(tot_frames - len(labels))]
